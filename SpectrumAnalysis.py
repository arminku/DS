...
class Soundfile:
    ...
    def subplot_Data_Norm(self, pltIdx, StartValue, EndValue, combined):  
        if not combined:     
            ...
            plt.ylabel('x(t)',size='large')
            plt.plot(self.X, self.Data_Mono_Norm,c= self.Color,linewidth=1, )
            plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
            plt.title('Rohdaten '+ self.Title,fontweight='bold',size='large')
            plt.xlim(StartValue,EndValue)
        else:
            plt.plot( self.X, self.Data_Mono_Norm,c= self.Color, alpha=0.5,linewidth=1)
    
    def subplot_Data_FFT(self, pltIdx, StartValue_X, EndValue_X,StartValue_Y,EndValue_Y, combined ):
        if not combined: 
            plt.subplot(3, 1, pltIdx)
            plt.ylabel('|X|',size='large')
            plt.plot(self.X_FFT, self.Data_FFT,c= self.Color,linewidth=1)
            plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
            plt.title('Betragsspektrum '+ self.Title,fontweight='bold',size='large')
            plt.xlim(StartValue_X, EndValue_X)
            plt.ylim(StartValue_Y, EndValue_Y)
        else:
            plt.plot( self.X_FFT, self.Data_FFT,c= self.Color, alpha=0.5,linewidth=1)

    def subplot_Data_FFT_Dif(self, pltIdx, StartValue, EndValue, Data):
        plt.subplot(2, 1, pltIdx)
        plt.ylabel('|X|',size='large')
        plt.title('Störfrequenzen bei '+ self.Title+ ' im Bereich von {}-{}Hz'.format(StartValue,EndValue),fontweight='bold',size='large')
        plt.plot(self.X_FFT, self.Data_FFT-Data,c= self.Color)
        plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
        plt.xlim(StartValue, EndValue)
    ...
    def subplot_Spectrogram(self, pltIdx, StartValue, EndValue, Data):       
        ...
        plt.title("Spectrogram im Frequenzbereich {}-{}Hz".format(StartValue,EndValue),fontweight='bold',size='large')
        plt.xlabel('Time [sec]',size='large')
        plt.ylabel('Frequency [Hz]',size='large')
       

def plotData (Soundfiles, StartValue, EndValue, combined):    #plot Data
    if not combined:
        fig, axs = plt.subplots(3, figsize=(16, 8), facecolor='#dddddd')
        ...
        fig=plt.figure(figsize=(16, 5), facecolor='#dddddd')
        Soundfiles[0].subplot_Data_Norm(1,StartValue,EndValue, combined)
        Soundfiles[1].subplot_Data_Norm(2,StartValue,EndValue, combined)
        Soundfiles[2].subplot_Data_Norm(3,StartValue,EndValue, combined) 
        plt.xlabel('time [s]',size='large')
        plt.ylabel('x(t)',size='large')
        plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
        plt.title('Rohdaten Signale kombiniert',fontweight='bold',size='large')
        fig.tight_layout()
        fig.subplots_adjust(top=0.9)
        plt.xlim(StartValue, EndValue)
        plt.show()
...
def plotDataFFT (Soundfiles,StartValue_X, EndValue_X,StartValue_Y,EndValue_Y, combined):    #plot Data
    if not combined:
        ...
        plt.xlabel('frequency [Hz]',size='large')
        fig.tight_layout()
        fig.subplots_adjust(top=0.9)
        plt.show()
    else:
        ...
        plt.ylabel('|X|',size='large')
        plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
        plt.title('Betragsspektrum Signale kombiniert',fontweight='bold',size='large')
        fig.tight_layout()
        fig.subplots_adjust(top=0.9)
        plt.xlim(StartValue_X, EndValue_X)
        plt.ylim(StartValue_Y, EndValue_Y)
...
def plotDataFFT_diff(Soundfiles,StartValue, EndValue):
    fig, axs = plt.subplots(2, figsize=(16, 8), facecolor='#dddddd')
...
def plot_Spectrogram(Soundfiles,StartValue, EndValue):   
    fig, axs = plt.subplots(2, figsize=( 16,10), facecolor='#dddddd')
...
def plot_amplitude_maxfreq(SoundfileEngine2:Soundfile,SoundfileNominal:Soundfile):
    ...
    plt.xlim(0,20)
    plt.title("Verlauf der aufbauenden Frequenz in Engine Faulty2",fontweight='bold',size='large')
    plt.xlabel('Time [sec]',size='large')
    plt.ylabel('Power/Frequency [dB/Hz]',size='large')
    plt.grid(b=True, which='major', color='black', alpha=0.5, linestyle='dashdot', lw=1)
    fig.tight_layout()    
    fig.subplots_adjust(top=0.9)
    plt.show()
